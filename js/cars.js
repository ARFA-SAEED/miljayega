fetch("miljaega-cars.json")
  ...
  .then((data) => {
    ...
    const cardsContainer = document.getElementById("mycards"); // ✅ store reference

    for (let i = 0; i < data.length; i++) {
      const card = data[i]; // ✅ use different name
...
<div class="card overflow-hidden rounded-lg shadow border border-gray-200 bg-white transform transition-transform duration-300 ease-in-out hover:scale-[1.02]">
  ...
  <div class="p-4 flex flex-col flex-grow">
    ...
    <h3 class="text-md font-semibold text-gray-800 mt-2">${card.title}</h3>
    <p class="text-xl font-bold text-gray-900 mt-2">${card.price}</p>
    <p class="text-sm text-gray-500 mt-1">${card.address}</p>
    <div class="flex-grow"></div>
    <p class="text-xs text-gray-400 mt-5 text-right">${card.day}</p>
  </div>
</div>
`;
    }
